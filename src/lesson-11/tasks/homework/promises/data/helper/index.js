...
const getExecutionTime = () => {
    const maxExecutionTime = 5 * 1000;
    return Math.floor(Math.random() * (maxExecutionTime - 1000)) + 1000;
};

const runner = (_f, ...args) => {
    const executionTime = getExecutionTime();
    console.log(`${_f.name} start${ (args.length > 0) ? '. Args: ' + args : ''}. Expected execution time ${executionTime}`);
    const startTime = new Date().getTime();
    return new Promise(resolve => {
        setTimeout(() => {
                console.log(`${_f.name} finished. Execution time ${new Date().getTime() - startTime}`);
                resolve(_f(...args));
            }
            , executionTime)
    })
...
export {validator, getEntity, toInt, logAndReturn, isNumInRange, getValueOrNull, runner, getExecutionTime};
