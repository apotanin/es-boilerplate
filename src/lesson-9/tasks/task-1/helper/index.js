...
let validator = ({id, firstName, lastName, age, sex}) => {
    ...
    if (age === undefined) throw new Error('age should be defined');
    if (typeof age !== 'number') throw new Error('age should be a number');
    if (age < 0) throw new Error('age should be a positive number');

    if (sex === undefined) throw new Error('sex should be defined');
    if (sex !== 'male' && sex !== 'female') throw new Error('sex should be male or female');
};
